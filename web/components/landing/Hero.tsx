      ...
      <div className="mx-auto grid max-w-[1100px] items-center gap-x-14 gap-y-12 px-5 pb-20 pt-16 sm:px-8 sm:pt-24 lg:grid-cols-[1.02fr_0.98fr]">
        <div>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-x-5 gap-y-3">
            <Link
              href="/run"
              className={`rounded-md border border-iris/40 bg-iris-dim px-4 py-2 font-mono text-[0.8125rem] text-iris-fg transition-colors hover:border-iris/70 ${FOCUS}`}
            >
              Run an audit in the browser →
            </Link>
            <Link href="/report" className={`rounded-sm text-[0.8125rem] text-secondary underline-offset-4 transition-colors hover:text-fg hover:underline ${FOCUS}`}>
        ...
        <Link
          ...
          className={`group block rounded-lg border border-hairline bg-surface/30 p-6 transition-colors hover:border-line sm:p-7 ${FOCUS}`}
        >
          ...
          <div className="space-y-3">
            {SIGNALS.map(([label, status], index) => (
              <div key={label} className="grid grid-cols-[1fr_auto] items-center gap-4 border-t border-hairline pt-3">
                <span className="text-[0.8125rem] text-secondary">{label}</span>
                <span className={`font-mono text-[0.6875rem] ${index === 3 ? "text-warn-fg" : "text-iris-fg"}`}>
                  {status}
                </span>
              </div>
            ))}
          </div>
          <span className="mt-6 inline-block font-mono text-[0.75rem] text-iris-fg transition-transform group-hover:translate-x-0.5">
