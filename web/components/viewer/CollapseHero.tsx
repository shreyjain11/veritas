...
function MetricBar({
  ...
  return (
    <div className="relative h-7">
      ...
      {/* bar */}
      <div
        className={cn("absolute top-1/2 h-3 -translate-y-1/2 rounded-sm transition-[width] duration-700 ease-out motion-reduce:transition-none", fill)}
        style={{ left: `${left}%`, width: animate ? `${width}%` : "0%" }}
      />
      {/* endpoint cap */}
      <div
        className={cn("absolute top-1/2 h-3 w-0.5 -translate-y-1/2 transition-[left] duration-700 ease-out motion-reduce:transition-none", cap)}
        style={{ left: `calc(${animate ? end : zero}% - 1px)` }}
        aria-hidden
      />
    </div>
  );
...
export function CollapseHero({ report }: { report: AuditReport }) {
  ...
  return (
    <div className="rounded-md border border-hairline bg-surface/60 px-5 py-5">
      ...
      <div className="relative">
        ...
        <div className="grid grid-cols-[4.5rem_1fr] items-center gap-x-3 gap-y-1">
          ...
          <MetricBar traced={reported} scale={scale} tone="warn" animate={animate} />
          <span className="font-mono text-[0.6875rem] text-iris-fg">honest</span>
          <MetricBar traced={honest} scale={scale} tone="iris" animate={animate} />
        </div>
      </div>
